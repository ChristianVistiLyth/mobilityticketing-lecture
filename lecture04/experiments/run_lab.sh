...
# match the sections of lecture04/README.md. Expected failures must
...
set -u
cd "$(dirname "$0")/../.."
dir=lecture04/experiments
migrations=lecture04/migrations
...
echo "(in lectures 1-3, leaving out the init/ files that only create the column)"
grep -rl "product_code" lecture01 lecture02 lecture03 --include=*.sql | grep -v "/init/" | sort
